...
# load average calculator freqence
freq_data = np.load('calculator_freq.npy')
sound = 0
...
while stream.is_active():
    ...
    # Clear Distance and sound
    distance = []

    # Run the FFT on the windowed buffer
    fft = np.fft.rfft(buf * window)
    # plt.figure()
    # plt.plot(fft[imin:imax])
    # plt.show()
    # Get frequency of maximum response in range
    freq = (np.abs(fft[imin:imax]).argmax() + imin) * FREQ_STEP
    if freq < 1500 and freq > 400:
        # Get note number and nearest note
        
        for i in range(len(freq_data)):
        	distance.append(abs(freq - freq_data[i]))
        sound_loop = np.argmin(distance) + 1
        if sound_loop != sound:
        	sound = sound_loop
        	print(sound)
    else:
